...
SELECT 
    v.vendor_name, 
    p.product_name, 
    5 * COUNT(c.customer_id) AS total_sales
...
VALUES ('Apple Pie', 'Small', 'unit', CURRENT_TIMESTAMP);
...
WHERE product_name = 'Apple Pie' AND product_size = 'Small';
...
SET pu.current_quantity = COALESCE(
    (SELECT vi.quantity FROM vendor_inventory vi WHERE vi.product_id = pu.product_id ORDER BY vi.date DESC LIMIT 1),
    0
)
WHERE EXISTS (
    SELECT 1 FROM vendor_inventory vi WHERE vi.product_id = pu.product_id
);
